...
b1 <- round(beta0hat <- mean(y[x == 0]), 2)
...
# Estimación con modelo de regresión
coef(mef)
mean(datos.s$y[datos.s$x == 0])
mean(datos.s$y[datos.s$x == 1])

# Media poblacional
mean(datos$y[datos$x == 0])
mean(datos$y[datos$x == 1])
